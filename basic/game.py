...
if True:
    ...
    while True:# 반드시 내부에 break가 있어야 한다
        - 2-1 "게임 제목을 입력하시오, 단 20자 이내로 입력 가능합니다." 문구 출력
            -> 매번 입력을 대기할때마다 해당 프럼프트를 출력하겠다
        # 사용자가 엔터키를 칠때까지 코드를 블럭하고 있다
        tmp = input("게임 제목을 입력하시오, \n\ 단 {} 이내로 입력 가능합니다. => .format(GAME_TITLE_LEN_MAX)") .strip()
        ...
        if not tmp: # 2-3 아무것도 입력하지 않고 엔터를 치면
            ...
            print("정확하게 입력하세요")
            pass
        elif len(tmp)>GAME_TITLE_LEN_MAX : # 2-4 20자이상 입력하면 
    ...
    # 3단계
    # gameTitle 이 정의된 곳 -> while 안에 else 안에서 정의
    # while 밖에서도 사용이 가능하다?
